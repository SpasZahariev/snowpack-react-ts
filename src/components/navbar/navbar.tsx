  ...
  return (
    ...
    >
      <div className="px-4 py-1 flex justify-between items-center">
        {/* Logo */}
        <span onClick={handleHome} className="cursor-pointer group" title="Back to top">
          <svg
        ...
        <div className="hidden md:flex items-center gap-4">
          {navItems.map((item) => (
            <span
              key={item.label}
              onClick={item.action}
              className="text-text cursor-pointer hover:bg-surface0 hover:text-pink px-3 py-2 rounded-md transition-colors"
            >
              {item.label}
            </span>
          ))}
